	...
	if not registeredCommands then
					...
					for i = 1, #Config.CookLocations, 1 do
						if #(coords - Config.CookLocations[i][2]) <= 3.0 then
							...
							if Config.LocationFoods[i][foodBeingCooked] ~= nil then
								...
								zoneType = Config.CookLocations[i][4]
								break
							end
						end
					end
		...
		RegisterCommand('cookfood', function(source, args, rawCommand)
			...
			if amount ~= nil then
				...
				TriggerEvent("DoLongHudText","Not a valid amount",2, 5000)
			end--]]
			if not startedFoodProcess then
				if #args == 1 then
					local ped = GetPlayerPed(-1)
	...
	if hasCollectedFood then
		if cookAmount > 0 then
			if #(coords - Config.CookLocations[cookLocation][2]) <= 1.5 then
				...
				QBCore.Functions.Notify("You are not in the correct spot to cook " .. foodBeingCooked, "error", 7500)
			end
		else
			QBCore.Functions.Notify("Not a valid amount", "error", 7500)
		end
	else
		QBCore.Functions.Notify("You need to collect your other " .. foodBeingCooked .. " first", "error", 7500)
	end
end
...
function startCocktailShake()
	...
	if totalErrors > 0 then
		...
		if skillErrors > 0 then
			if errorString ~= "" then	
				errorString = errorString .. 'and for messing up while preparing the ' .. foodBeingCooked .. '.'
			else
				errorString = 'You received lower quality ' .. foodBeingCooked .. ' due to messing up while preparing the ' .. foodBeingCooked .. '.'
			end
		end
		if shakeErrors > 0 then
			if errorString ~= "" then	
				...
				errorString = 'You received lower quality ' .. foodBeingCooked .. ' due to messing up the shaking of the cocktail'
			end
		end
...
function getFood()
	local ped = GetPlayerPed(-1)
	local coords = GetEntityCoords(ped, true)
	if #(coords - Config.CookLocations[cookLocation][2]) <= 1.0 then
		receiveFood(totalErrors)
		if totalErrors > 0 then
			local errorString = ""
			if timeError > 0 then
				errorString = 'You received lower quality ' .. foodBeingCooked .. ' due to cooking improper time'
			end
			if heatErrors > 0 then
				if errorString ~= "" then	
					errorString = errorString .. ', cooking at wrong temperature for extened time'
				else
					errorString = 'You received lower quality ' .. foodBeingCooked .. ' due to cooking at wrong temperature for extened time'
				end
			end
			if skillErrors > 0 then
				if errorString ~= "" then	
		...
		QBCore.Functions.Notify("You are not in the correct spot to collect your " .. foodBeingCooked, "error", 7500)
	end
end
	...
	Citizen.CreateThread(function()
		...
		while not placedIngredients and (GetGameTimer() - startTime) <= 60000 and not reset do
			...
			if #(Config.CookLocations[cookLocation][2] - coords) <= 2.5 then
				DrawText3Ds(Config.CookLocations[cookLocation][2].x, Config.CookLocations[cookLocation][2].y, (Config.CookLocations[cookLocation][2].z + 1), "Cook amount is currently ~y~" .. cookAmount .. " " .. foodBeingCooked .. "(s)")
			...
			if IsControlJustPressed(0, 49) and #(Config.CookLocations[cookLocation][2] - coords) <= 0.5 then
				if hasRequiredIngredients() then
					...
					TriggerEvent('chatMessage', 'RESTAURANTS: ', 3, "Minimum cook time for " .. cookAmount .. " " .. foodBeingCooked .. "(s) is " .. minCookTime .. " seconds")
		...
		while not cookingFood and placedIngredients and not reset do
			...
			local coords = GetEntityCoords(ped, true)
			if #(Config.CookLocations[cookLocation][1] - coords) <= 2.5 then
				DrawText3Ds(Config.CookLocations[cookLocation][1].x, Config.CookLocations[cookLocation][1].y, Config.CookLocations[cookLocation][1].z, "~o~"..cookAmount.." " .. foodBeingCooked .. "(s)~s~".." / ~g~"..foodTemperature.."° Fahrenheit~s~ / ~y~"..elapsedCookTime.."(s)~s~")
			end
			if #(Config.CookLocations[cookLocation][1] - coords) <= 2.5  then
	...
	Citizen.CreateThread(function()
		while cookingFood or not hasCollectedFood and not reset do
			...
			if displayText and cookingFood then
				DrawText3Ds(Config.CookLocations[cookLocation][1].x, Config.CookLocations[cookLocation][1].y, Config.CookLocations[cookLocation][1].z, "~o~"..pounds.." " .. foodBeingCooked .. "(s)~s~".." / ~g~"..foodTemperature.."° Fahrenheit~s~ / ~y~"..elapsedCookTime.."(s)~s~")
			end
			...
			if not cookingFood then
				local ped = GetPlayerPed(-1)
				local coords = GetEntityCoords(ped, true)
				if #(coords - Config.CookLocations[cookLocation][2]) <= 0.5 then
					DrawText3Ds(Config.CookLocations[cookLocation][2].x, Config.CookLocations[cookLocation][2].y, Config.CookLocations[cookLocation][2].z, "Press ~g~F~s~ to to collect your " .. foodBeingCooked)
				end
			end
			if IsControlJustPressed(0, 49) and not cookingFood then
				getFood()
			end
...
function startCookLoop()
	...
	Citizen.CreateThread(function()
		...
		QBCore.Functions.Notify("You have stopped cooking your ".. foodBeingCooked .. ". Go to the prep location to collect your food", "error", 7500)
	end)
end
